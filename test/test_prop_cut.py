...
class TestCutProperty(unittest.TestCase):
    @given(
        data=st.text(min_size=1, max_size=50, alphabet=st.characters(blacklist_characters=['\xa0', '\u1234'])), # blacklist chars represented in more than 1 byte
    ...
    def test_cut_byte_range(self, data, start, end):
        ...
        input_str = '\n'.join([data] * 25)  # Repeat the data to form multiple lines
    ...
    @given(
        ...
        data=st.text(min_size=1, max_size=50, alphabet=st.characters(blacklist_characters=['\xa0', '\u1234', '\n'])),
